...
import { Alert, Button, Card, Col, Container, Form, Modal, Row, Toast } from "react-bootstrap";
import BuildsUtil from "../assets/BuildsUtil";
import { useForm, SubmitHandler, useFieldArray, UseFieldArrayReturn } from "react-hook-form"
...
function BuildBuilder() {
    const armourParts = ["Helm", "Shoulders", "Coat", "Gloves", "Leggings", "Boots"];
    const accessoryParts = ["Amulet", "Ring", "Ring", "Accessory", "Accessory", "Back Item"];
    ...
    const handleSetAllArmorStat = (stat: string) => {
        armourParts.forEach((part, index) => {
    ...
    const traitLineOptions = traitLinesAvailable.map((id) => allTraitLines.find((t) => t.id === id)).filter(Boolean)
    ...
    const primaryWeapons = useFieldArray({
        control,
        name: 'primaryWeapons'
    })
        const secondaryWeapons = useFieldArray({
        control,
    ...
    const getHandsUsed = (weapons: Weapon[]) => {
        return weapons.reduce((total, weapon) => {
            const typeMeta = BuildsUtil.getWeapons().find(w => w.type === weapon.type);
    ...
    function buildReplacer(key: string, value: any) {
        ...
        if (key === "secondaryWeapons" || key === "primaryWeapons") {
            for (let index = 0; index < value.length; index++) {
                ...
                if (BuildsUtil.getWeapons().find(w => w.type === weapon.type)?.hands === 1) {
    ...
    useEffect(() => {
        register("traitLine1", {
            validate: (value) =>
            value && value.length === 3 && value[0] != null && value[1] != null && value[2] != null && value.every(Boolean)
                ? true
    ...
    const renderWeapons = (
        weapons: UseFieldArrayReturn<Build, "primaryWeapons" | "secondaryWeapons", "id">,
        fieldName: "primaryWeapons" | "secondaryWeapons",
        handsUsed: number
        ) => {
        return (
            <>
            {weapons.fields.map((field, index) => (
                ...
                <Form.Group>
                    ...
                    <Form.Select defaultValue={""} {...register(`${fieldName}.${index}.type`, {required:"Empty weapon slots not currently supported. Please either remove this weapon or set a weapon type."})}>
                    <option hidden disabled value="">Select Weapon Type</option>
                    ...
                    <Form.Select defaultValue={""} {...register(`${fieldName}.${index}.stat`, {required:"Please select a weapon stat"})}>
                    <option hidden disabled value="">Select Stat</option>
                    ...
                    <Form.Control
                    type="number"
                    {...register(`${fieldName}.${index}.sigils.0`, { valueAsNumber: true })}
                    placeholder="Sigil 1 ID"
                    />
                    {weapons.fields.length == 1 && handsUsed == 2 && (
                    <Form.Control
                    type="number"
                    {...register(`${fieldName}.${index}.sigils.1`, { valueAsNumber: true })}
            ...
            <p><strong>Hands used:</strong> {handsUsed}/2</p>

            {handsUsed < 2 &&  weapons.fields.length < 2 && (
                <Button
                variant="secondary"
                onClick={() => {
                    weapons.append({ type: "", stat: "", sigils: [] });
                }}
                >
                Add Weapon
                </Button>
    ...
    return <Container style={{paddingTop:50, color:"white"}}>
        ...
        <Card.Body>
            <Row>
                ...
                <Form.Select {...register("class", {required: "Please select a profession"})} onChange={handleProfessionSelected} defaultValue={""}>
                    <option hidden disabled value="">Select Profession</option>
                    {professions.map((profession, index) => 
                        <option key={index} value={profession}>{profession}</option>
                    )}
                </Form.Select>
                {errors.class && <Alert variant="danger">{errors.class.message}</Alert>}
                </Col>
                <Col>
                <Form.Label>Specialization</Form.Label>
                <Form.Select value={watch("spec")} {...register("spec", {required: "Please select a specialization"})}>
                    <option hidden disabled value="">Select Specialization</option>
                    {specs.map((spec, index) => 
                        <option key={index} value={spec}>{spec}</option>
                    )}
                </Form.Select>
                {errors.spec && <Alert variant="danger">{errors.spec.message}</Alert>}
        </Col>
        </Row>
        <Row>
        <Col>
        ...
        <Form.Control as="textarea" rows={3} {...register("variations")} />
        </Col>
        </Row>
        <Row>
        <Col>
        ...
        <Row>
        {renderWeapons(primaryWeapons, "primaryWeapons", primaryHandsUsed)}
        {showSecondary && (
        <>
            <Row>
            <Form.Label>Secondary Weapon Set</Form.Label>
            </Row>
            {renderWeapons(secondaryWeapons, "secondaryWeapons", secondaryHandsUsed)}
        </>
        )}
        </Row>
        </Card.Body>
        ...
        <Form.Group>
        <Form.Label><strong>Set All Armour</strong></Form.Label>
            <div className="d-flex gap-2">
                <Form.Select defaultValue={""} onChange={(e) => handleSetAllArmorStat(e.target.value)} >
                    <option hidden disabled value="">Select Stat</option>
        ...
        {armourParts.map((part, index) => (
            <div key={index}>
                ...
                <Form.Select defaultValue={""} {...register(`gear.${index}.stat`, {required:"Please select an armour stat"})}>
                    <option hidden disabled value="">Select Stat</option>
                    {BuildsUtil.getArmourStats().map((stat: string) => (
                        <option key={stat} value={stat}>{stat}</option>
                    ))}
                ...
                <Form.Control
                    type="number"
                    placeholder="Rune ID"
                    {...register(`gear.${index}.rune`, { valueAsNumber: true })}
                    className="mt-1"
                />
                <input type="hidden" value={part} {...register(`gear.${index}.type`)} />
            </div>
        ))}
        </Col>
        <Col>
        ...
        <Form.Control
        type="number"
        {...register("relic", { valueAsNumber: true,  required: "Please enter a relic" })}
        ...
        <Form.Group>
        <Form.Label><strong>Set All Accessories</strong></Form.Label>
        <Form.Select
            defaultValue={""} 
        ...
        {accessoryParts.map((item, index) => (
            <div key={index}>
                ...
                <Form.Select defaultValue={""} {...register(`accessories.${index}.stat`, {required:"Please select an accessory stat"})}>
                    <option hidden disabled value="">Select Stat</option>
                    {BuildsUtil.getAccessoryStats().map((stat: string) => (
                        <option key={stat} value={stat}>{stat}</option>
                    ))}
                ...
                <input type="hidden" value={item} {...register(`accessories.${index}.type`)} />
            </div>
        ))}
        </Col>
        </Row>
        ...
        <Col key={i}>
        <Form.Group>
            <Form.Select value={watch(`traitLines.${i}`) || ""} {...register(`traitLines.${i}`, {setValueAs: (v) => Number(v), required:"Please select a trait line"})}>
            <option hidden disabled value="">Select Trait Line</option>
        ...
        onSelect={({ id, tier }) => {
            ...
            current[tier] = id;
            setValue("traitLine1", current, { shouldValidate: true });
        }}
        />
        {errors.traitLine1 && (
            <Alert variant="danger">{errors.traitLine1.message}</Alert>
        )}
        </Row>
        )}
        {traitLines?.[1] && (
        ...
        onSelect={({ id, tier }) => {
            ...
            current[tier] = id;
            setValue("traitLine2", current, { shouldValidate: true });
        }}
        />
        {errors.traitLine2 && (
            <Alert variant="danger">{errors.traitLine2.message}</Alert>
        )}
        </Row>
        )}
        {traitLines?.[2] && (
        ...
        onSelect={({ id, tier }) => {
            ...
            current[tier] = id;
            setValue("traitLine3", current, { shouldValidate: true });
        }}
        />
        {errors.traitLine3 && (
            <Alert variant="danger">{errors.traitLine3.message}</Alert>
        )}
        </Row>
        )}
        </Row>
        ...
        <Row>
            ...
            <Form.Select defaultValue={""} {...register("utilitySkills.0", { required: isRevenant ? "Please select a legend" : false })}>
            <option hidden disabled value="">Select Legend</option>
            {BuildsUtil.getRevenantLegends().map((legend: { id: number, name: string }) => (
                <option key={legend.id} value={legend.id}>{legend.name}</option>
            ))}
            </Form.Select>
            {errors.utilitySkills?.[0]?.message && isRevenant && (
                <Alert variant="danger">{errors.utilitySkills[0].message}</Alert>
            )}
            </Col>
            <Col>
            <Form.Label>Legend 2</Form.Label>
            <Form.Select defaultValue={""} {...register("utilitySkills.1", { required: isRevenant ? "Please select a legend" : false })}>
            <option hidden disabled value="">Select Legend</option>
            {BuildsUtil.getRevenantLegends().map((legend: { id: number, name: string }) => (
                <option key={legend.id} value={legend.id}>{legend.name}</option>
            ))}
            ...
            {errors.utilitySkills?.[1]?.message && isRevenant && (
                <Alert variant="danger">{errors.utilitySkills[1].message}</Alert>
            )}
            </Col>
        </Row>
        ) : (
        <Row>
            ...
            <Form.Control
            type="number"
            {...register("healSkill", { valueAsNumber: true,  required: !isRevenant ? "Empty skills are not currently supported. Please enter a skill ID and add alternatives to notes." : false })}
            ...
            <Form.Control
            type="number"
            {...register("utilitySkills.0", { valueAsNumber: true, required: !isRevenant ? "Empty skills are not currently supported. Please enter a skill ID and add alternatives to notes." : false })}
            ...
            <Form.Control
            type="number"
            {...register("utilitySkills.1", { valueAsNumber: true, required: !isRevenant ? "Empty skills are not currently supported. Please enter a skill ID and add alternatives to notes." : false })}
            placeholder="Utility Skill ID"
            />
            {errors.utilitySkills?.[1]?.message && !isRevenant && (
                <Alert variant="danger">{errors.utilitySkills[1].message}</Alert>
            ...
            <Form.Control
            type="number"
            {...register("utilitySkills.2", { valueAsNumber: true, required: !isRevenant ? "Empty skills are not currently supported. Please enter a skill ID and add alternatives to notes." : false })}
            placeholder="Utility Skill ID"
            />
            {errors.utilitySkills?.[2]?.message && !isRevenant && (
                <Alert variant="danger">{errors.utilitySkills[2].message}</Alert>
            )}
            </Col>
            <Col>
            ...
            <Form.Control
            type="number"
            {...register("eliteSkill", { valueAsNumber: true, required: !isRevenant ? "Empty skills are not currently supported. Please enter a skill ID and add alternatives to notes." : false })}
            placeholder="Elite Skill ID"
            />
            {errors.eliteSkill?.message && !isRevenant && (
                <Alert variant="danger">{errors.eliteSkill.message}</Alert>
            )}
            </Col>
        </Row>
        )}
        ...
        <Modal data-bs-theme="dark" show={showModal} onHide={() => setShowModal(false)} centered>
